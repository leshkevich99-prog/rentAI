    ...
    <div className="bg-black min-h-screen pt-24 pb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
            <Link to="/fleet" className="inline-flex items-center text-gray-400 hover:text-gold-400 transition-colors uppercase text-xs font-bold tracking-widest">
                <ChevronLeft size={16} className="mr-2" /> Назад в автопарк
            </Link>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-8">
                        <div className="bg-dark-900 border border-white/5 p-4 rounded-xl text-center">
                            ...
                            <p className="text-gray-500 text-xs uppercase">Л.С.</p>
                        </div>
                        <div className="bg-dark-900 border border-white/5 p-4 rounded-xl text-center">
                            <Wind className="w-6 h-6 text-gold-500 mx-auto mb-2" />
                            <p className="text-white font-bold text-lg">{car.specs.zeroToSixty}с</p>
                            <p className="text-gray-500 text-xs uppercase">0-100 км/ч</p>
                        </div>
                        <div className="bg-dark-900 border border-white/5 p-4 rounded-xl text-center">
                            <Gauge className="w-6 h-6 text-gold-500 mx-auto mb-2" />
                            <p className="text-white font-bold text-lg">{car.specs.maxSpeed}</p>
                            <p className="text-gray-500 text-xs uppercase">Км/ч</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <h1 className="font-serif text-4xl md:text-5xl text-white mb-4 leading-tight">{car.name}</h1>
                    <p className="text-2xl font-bold text-gold-400 mb-8">
                        {car.pricePerDay.toLocaleString()} BYN <span className="text-sm font-normal text-gray-500 text-white">/ сутки</span>
                    </p>

                    <div className="prose prose-invert mb-8">
                        <h3 className="text-white font-serif text-xl mb-4">Об автомобиле</h3>
                        <p className="text-gray-400 leading-relaxed">
                            {car.description || "Описание для данного автомобиля скоро появится. Это один из лучших представителей своего класса, сочетающий комфорт, скорость и безупречный стиль."}
                        </p>
                    </div>
                    ...
                    <div className="bg-dark-900 border border-white/5 rounded-xl p-6 mb-8">
                         <div className="flex items-center gap-3 mb-4">
                             ...
                             <h3 className="text-white font-bold uppercase text-sm tracking-wider">Система скидок</h3>
                         </div>
                         
                         <div className="space-y-3">
                             {sortedRules.map((rule, idx) => (
                                 <div key={idx} className="flex justify-between items-center border-b border-white/5 pb-2 last:border-0 last:pb-0">
                                     <span className="text-gray-400 text-sm">Более {rule.days} дней</span>
                                     <span className="text-green-400 font-bold">-{rule.discount}%</span>
                                 </div>
                             ))}
                             {sortedRules.length === 0 && <p className="text-gray-500 text-sm">Скидки уточняйте у менеджера.</p>}
                         </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                         ...
                         >
                            {car.available ? 'Забронировать сейчас' : 'Временно недоступно'}
                         </button>
